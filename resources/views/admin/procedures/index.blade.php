                    ...
                    <tr>
                        ...
                        <td>
                            <div class="action-group">
                                <a href="{{ route('admin.procedures.show', $procedure) }}" class="btn-dental btn-sm">عرض</a>
                                <a href="{{ route('admin.procedures.edit', $procedure) }}" class="btn-dental btn-sm btn-primary">تعديل</a>
                                <button type="button" class="btn-dental btn-sm btn-danger" data-bs-toggle="modal" data-bs-target="#deleteModal{{ $procedure->id }}">
                                    حذف
                                </button>
                            </div>
                            
                            <!-- Delete Modal -->
                            <div class="modal fade" id="deleteModal{{ $procedure->id }}" tabindex="-1" aria-labelledby="deleteModalLabel{{ $procedure->id }}" aria-hidden="true">
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            <h5 class="modal-title" id="deleteModalLabel{{ $procedure->id }}">تأكيد الحذف</h5>
                                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                        </div>
                                        <div class="modal-body">
                                            هل أنت متأكد من رغبتك في حذف الإجراء <strong>{{ $procedure->name }}</strong>؟
                                        </div>
